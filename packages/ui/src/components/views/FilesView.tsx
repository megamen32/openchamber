...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const selectedFileReadOptions = React.useMemo(
    ...
    [mode, root, selectedFileIsOutsideWorkspace, selectedOutsideFileGrant],
  );
  ...
  const handleRevealPath = React.useCallback((targetPath: string) => {
    ...
    void files.revealPath(targetPath, fileWorkspaceOptions).catch(() => {
  ...
  const handleDownloadFile = React.useCallback((targetPath: string) => {
    if (!files.downloadFile) return Promise.resolve();
    return files.downloadFile(targetPath, fileWorkspaceOptions);
  }, [fileWorkspaceOptions, files]);
  ...
  const loadDirectory = React.useCallback(async (dirPath: string) => {
    ...
    const listPromise = files.listDirectory
      ? files.listDirectory(normalizedDir, fileWorkspaceOptions).then((result) => result.entries.map((entry) => ({
        name: entry.name,
        path: entry.path,
  ...
  }, [fileWorkspaceOptions, files, mapDirectoryEntries]);
  ...
  const handleDialogSubmit = React.useCallback(async (e?: React.FormEvent) => {
    ...
    if (activeDialog === 'createFile') {
      ...
      await files.writeFile(newPath, '', fileWorkspaceOptions)
        .then(async (result) => {
          if (result.success) {
    ...
    if (activeDialog === 'createFolder') {
      ...
      const prefix = parentPath ? `${parentPath}/` : '';
      const newPath = normalizePath(`${prefix}${dialogInputValue.trim()}`);
      await files.createDirectory(newPath, fileWorkspaceOptions)
        .then(async (result) => {
          if (result.success) {
    ...
    if (activeDialog === 'rename') {
      ...
      await files.rename(oldPath, newPath, fileWorkspaceOptions)
        .then(async (result) => {
          if (result.success) {
    ...
    if (activeDialog === 'delete') {
      ...
      await files.delete(deletedPath, fileWorkspaceOptions)
        .then(async (result) => {
          if (result.success) {
  ...
  }, [activeDialog, dialogData, dialogInputValue, fileWorkspaceOptions, files, refreshDirectory, isMobile, removeOpenPathsByPrefix, root, selectedFile?.path, setSelectedPath, t]);
  ...
  const readFile = React.useCallback(async (path: string, options?: FileReadOptions): Promise<string> => {
    const requestOptions = root && !options?.directory ? { ...options, directory: root } : options;
    if (files.readFile) {
      const result = await files.readFile(path, requestOptions);
      return result.content ?? '';
    }

    const params = new URLSearchParams({ path });
    if (requestOptions?.allowOutsideWorkspace) {
      params.set('allowOutsideWorkspace', 'true');
    }
    if (requestOptions?.outsideFileGrant) {
      params.set('outsideFileGrant', requestOptions.outsideFileGrant);
    }
    if (requestOptions?.optional) {
      params.set('optional', 'true');
    }
    if (requestOptions?.directory) {
      params.set('directory', requestOptions.directory);
    }
    const response = await runtimeFetch(`/api/fs/read?${params.toString()}`, {
      // Avoid conditional requests (304 + empty body).
      cache: requestOptions?.optional ? 'no-store' : 'default',
    });
    if (!response.ok) {
  ...
  const readFileStat = React.useCallback(async (path: string, options?: FileReadOptions): Promise<FileStatSnapshot | null> => {
    if (files.statFile) {
      const requestOptions = root && !options?.directory ? { ...options, directory: root } : options;
      const result = await files.statFile(path, requestOptions);
      return {
        path: result.path,
  ...
  }, [files.statFile, openPaths, readFileStat, removeOpenPathsByPrefix, root]);
  ...
  const saveDraft = React.useCallback(async () => {
    ...
    try {
      ...
      const result = await files.writeFile(selectedFile.path, contentToWrite, fileWorkspaceOptions);
      if (!result?.success) {
  ...
  }, [draftContent, fileContent, fileWorkspaceOptions, files, isDirty, loadedFileLineEnding, loadedFilePath, readFileStat, selectedFile, t]);
      ...
      return (
        <li key={node.path} className="relative">
          ...
          <FileRow
            ...
            downloadFile={files.downloadFile ? handleDownloadFile : undefined}
  ...
  const saveDiagramXml = React.useCallback(async (path: string, xml: string) => {
    ...
    const result = await files.writeFile(path, xml, fileWorkspaceOptions);
    if (!result?.success) {
      toast.error(t('filesView.toast.writeFileFailed'));
      return false;
    }
    ...
    return true;
  }, [fileWorkspaceOptions, files, readFileStat, selectedFileReadOptions, t]);
  ...
  const imageAssetAuthKey = selectedFile?.path && isSelectedImage && !runtime.isDesktop && !isSelectedSvg
    ? `${selectedFile.path}|${selectedFileReadOptions.allowOutsideWorkspace ? 'outside' : 'workspace'}|${selectedFileReadOptions.outsideFileGrant ?? ''}|${root}`
    : '';
        ...
        {files.downloadFile && (
          withTooltip(t('filesView.editor.saveFile'),
            <Button
              ...
              size="sm"
              onClick={() => {
                void handleDownloadFile(selectedFile.path).catch((error) => {
                  console.error('Download failed:', error);
                  toast.error(t('sidebarFilesTree.toast.operationFailed'));
                });
              }}
