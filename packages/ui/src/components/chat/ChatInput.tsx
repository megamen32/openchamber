...
} from "@/components/chat/SessionGoalButton";
import type { Part } from "@opencode-ai/sdk/v2/client";
import { ComposerToolMenu } from "./ComposerToolMenu";
import {
  callAction,
  type CallableAction,
} from "@/lib/opencode/tool-call-client";
...
const INLINE_SKILL_TOKEN_PATTERN =
  /(^|\s)\/([a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?)/g;
const CHAT_DRAFT_PERSIST_DEBOUNCE_MS = 500;
const getChatDraftSnapshotSignature = (
  text: string,
  confirmedMentions: Iterable<string>,
): string => `${text}\u0000${[...confirmedMentions].sort().join("\u0000")}`;
...
const buildImagePasteInsertion = (
  pastedText: string,
  citationText: string,
): string => {
...
const getInsertedTextFromChange = (
...
const withInlineInsertionBoundaries = (
  content: string,
  before: string,
  after: string,
): string => {
...
const getRevertedPreview = (parts: Part[], fallback: string): string => {
  ...
  if (text) return text;
  const filePart = parts.find((part) => part.type === "file") as
    (Part & { filename?: string }) | undefined;
...
const RevertedMessageDock: React.FC<RevertedMessageDockProps> = React.memo(
  ({ sessionId, directory }) => {
    const { t } = useI18n();
    ...
    const revertedStateRef = React.useRef<RevertedMessageDockState>(
      EMPTY_REVERTED_MESSAGE_DOCK_STATE,
    );
    ...
    const handleRestore = React.useCallback(
      async (messageId: string) => {
        ...
        try {
          const nextMessage = userMessages.find(
            (message) => message.id > messageId,
          );
          if (nextMessage) {
            await revertToMessage(sessionId, nextMessage.id, {
              skipRedoPush: true,
            });
          } else {
...
const PermissionAutoAcceptButton = React.memo(
  function PermissionAutoAcceptButton(props: PermissionAutoAcceptButtonProps) {
    const { t } = useI18n();
    const {
      footerIconButtonClass,
      iconSizeClass,
...
const FocusModeButton = React.memo(function FocusModeButton(
  props: FocusModeButtonProps,
) {
  const { footerIconButtonClass, iconSizeClass, isExpandedInput, onToggle } =
    props;
  const { t } = useI18n();
      ...
      <div className="relative">
        {hasContent ? (
          ...
          >
            <Icon
              name="send-plane-2"
              className={cn(sendIconSizeClass, "-rotate-90")}
            />
          </button>
        ) : null}
        <button
...
const ChatInputComponent: React.FC<ChatInputProps> = ({
  ...
  const [mobileComposerExpanded, setMobileComposerExpanded] =
    React.useState(false);
  const [mobileTextareaFocused, setMobileTextareaFocused] =
    React.useState(false);
  ...
  const [mobileDictationActive, setMobileDictationActive] =
    React.useState(false);
  ...
  const [mobileOverlayHostBusy, setMobileOverlayHostBusy] =
    React.useState(false);
  ...
  const composerHandleTouchRef = React.useRef<{
    startY: number;
    fired: boolean;
  } | null>(null);
  ...
  const persistDraftImmediately = React.useCallback(
    (identity: ChatDraftIdentity | null, draft: string) => {
      if (!identity) return;
  ...
  const prevChatDraftIdentityRef = React.useRef<ChatDraftIdentity | null>(
    initialDraftIdentityRef.current,
  );
  React.useEffect(() => {
    const previousIdentity = prevChatDraftIdentityRef.current;
    const previousKey = previousIdentity
      ? getChatDraftIdentityKey(previousIdentity)
      : null;
    const currentKey = chatDraftIdentity
  ...
  React.useEffect(
    () =>
      subscribeChatDraftDeletion((deletedIdentity) => {
        const deletedKey = getChatDraftIdentityKey(deletedIdentity);
        lastPersistedDraftRef.current.set(
          deletedKey,
          getChatDraftSnapshotSignature("", []),
        );
        const currentIdentity = currentChatDraftIdentityRef.current;
        if (
          !currentIdentity ||
          getChatDraftIdentityKey(currentIdentity) !== deletedKey
        )
          return;
        clearPendingDraftPersist();
        skipNextDraftPersistRef.current = true;
  ...
  const handleComposerToolAction = React.useCallback(
    async (
      ...
      if (!action.supported) {
        toast.error(
          action.unsupportedReason ??
            "This action is unavailable in the current OpenCode runtime.",
        );
        return;
      }

      const directory = currentSessionDirectoryForSync ?? currentDirectory;
      let argumentsForCall: Record<string, unknown> = suppliedArguments ?? {};
      if (suppliedArguments) {
        // The action dialog collected typed arguments; never parse the main composer as MCP JSON.
      } else {
        if (action.kind === "task") {
          argumentsForCall = {
            prompt:
              message.trim() ||
              "Continue the requested task from the current session context.",
          };
        } else {
      ...
      try {
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : String(error ?? "Direct tool call failed"),
        );
      }
    },
    [
  ...
  const handleSubmit = async (options?: SubmitOptions) => {
    ...
    if (currentSessionId && !queuedOnly) {
      const dismissedQuestions =
        await sessionActions.dismissOpenQuestionsForSession(currentSessionId);
      ...
      if (commandName === "undo" && currentSessionId) {
        ...
        try {
          ...
          toast.error(
            error instanceof Error
              ? error.message
              : t("chat.chatInput.toast.craftGoalFailed"),
          );
        }
        return;
      } else if (
        ...
        try {
          await sessionActions.waitForConnectionOrThrow();
          const idea = normalizedCommand
            .replace(/^\/schedule-task\b/i, "")
            .trim();
          const visibleText = await renderMagicPrompt(
          ...
          toast.error(
            error instanceof Error
              ? error.message
              : t("chat.chatInput.toast.scheduleTaskFailed"),
          );
        }
        return;
      } else if (
  ...
  const handleDictationInsertAndSend = React.useCallback((text: string) => {
    ...
    const next = appendInlineText(
      textareaRef.current?.value ?? messageRef.current,
      text,
    );
  ...
  const adjustTextareaHeight = React.useCallback(
    (options?: { allowShrink?: boolean }) => {
      ...
      const nextHeight = Math.min(
        Math.max(scrollHeight, dictationHeight),
        maxHeight,
      );
  ...
  const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const pastedInsertedText = nativeInputEvent?.inputType?.startsWith(
      "insertFromPaste",
    )
  ...
  const selectedDraftProjectBranchesFetchedAt = useGitStore((s) =>
    selectedDraftProjectPath
      ? (s.directories.get(selectedDraftProjectPath)?.lastBranchesFetch ?? 0)
      : 0,
  );
  ...
  React.useEffect(() => {
    ...
    const isStale =
      ...
      Date.now() - selectedDraftProjectBranchesFetchedAt >
  ...
  const renderProjectLabelWithIcon = React.useCallback(
    (project: {
      ...
      const projectIconName = project.icon
  ...
  const handleMobileDictationActiveChange = React.useCallback(
    (active: boolean) => {
  ...
  React.useEffect(() => {
    ...
    return () => {
      window.removeEventListener(
        "oc:mobile-overlay-opened",
        handleOverlayOpened,
      );
      window.removeEventListener(
        "oc:mobile-overlay-closed",
        handleOverlayClosed,
      );
    };
  }, [isMobile]);
  React.useEffect(() => {
    ...
    if (mobileOverlayOpen) {
      if (
        ...
        Date.now() - lastMobileBlurAtRef.current < 800
      ) {
  ...
  React.useEffect(() => {
    if (!isMobile || isCapacitorApp() || typeof document === "undefined")
      return;
  ...
  const handleComposerHandleTouchStart = React.useCallback(
    (event: React.TouchEvent) => {
      const touch = event.touches.item(0);
      composerHandleTouchRef.current = touch
        ? { startY: touch.clientY, fired: false }
        : null;
    },
    [],
  );
  const handleComposerHandleTouchMove = React.useCallback(
    (event: React.TouchEvent) => {
  ...
  React.useLayoutEffect(() => {
    ...
    if (!newSessionDraftOpen || isMobileExpanded || !mobileTextareaFocused)
      return;
    const vv = window.visualViewport;
    const form = composerFormRef.current;
    if (!vv || !form) return;
    ...
    const track = () => {
      ...
      const top = Math.max(
        0,
        Math.floor(Math.min(vvBottom, layoutBottom) - form.offsetHeight),
      );
      if (top !== lastTop) {
        lastTop = top;
        form.style.top = `${top}px`;
  ...
  const mobileComposerHandle = React.useMemo(
    () =>
      isMobile ? (
        <div
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        window.dispatchEvent(
                          new CustomEvent("openchamber:dictation-toggle"),
                        );
                      }}
                      ...
                      >
                        ...
                        <Textarea
                          ...
                          onBlur={() => {
                            ...
                            if (
                              Date.now() < holdComposerFocusUntilRef.current
                            ) {
                              const ta = textareaRef.current;
                              if (ta) {
                                ta.focus();
                                window.setTimeout(() => {
                                  if (
                                    Date.now() <
                                      holdComposerFocusUntilRef.current &&
                          ...
                          style={{
                            ...
                            height:
                              !isComposerExpanded && textareaSize
              ...
              <div className="flex flex-col">
                {(() => {
                  ...
                  return (
                    <>
                      {projectRootBranchOption &&
                      matches(projectRootBranchOption.label) ? (
                        <>
                      ...
                      {selectedDraftDirectory &&
                      !selectedDraftBranchIsKnown &&
                      matches(selectedDraftBranchLabel ?? "")
                        ? renderRow(
                            selectedDraftDirectory,
                            selectedDraftBranchLabel,
                            "unknown-current",
